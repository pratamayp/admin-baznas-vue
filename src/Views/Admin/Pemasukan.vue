<template>
  <div>
    ...
    <b-card class="main-card mb-4">
      ...
      <b-table id="payment-table" small :fields="fields" :items="dataIncome" :hover="true" :filter="searchInput"
      ...
      <b-row>
        <b-col cols="2">
          <b-form-select v-model="perPage" :options="[5, 10, 25]" class="per-page" @change="fetchIncome">
          </b-form-select>
          data
        </b-col>
        <b-col cols="10">
      ...
      <template #default="{ hide }">
        ...
        <form @submit="onSubmit">
          <div class="form-group">
            ...
            <multiselect id="tx-user" v-model="$v.addForm.user_id.$model" :options="dataUsers"
              :custom-label="customName" placeholder="Pilih pengguna" :class="{
...
export default {
  ...
  methods: {
    ...
    fetchPayments() {
      this.$api
        ...
        .then((res) => {
          this.dataPayments = [];
          res.data.items.map((a) => {
            if (a.status === 1) this.dataPayments.push(a);
          });
        })
        .catch((err) => {
          alert(err);
    ...
    fetchAccounts() {
      this.$api
        ...
        .then((res) => {
          this.dataAccounts = [];
          res.data.items.map((a) => {
            if (a.status === 1) this.dataAccounts.push(a);
          });
        })
        .catch((err) => {
          alert(err);
    ...
    onAdd() {
      this.modalTitle = "Tambah Transaksi Pemasukan";
      this.isEdit = false;
    ...
    exportTx() {
      const exp = [];
      for (let i = 0; i < this.dataIncome.length; i++) {
        let category, payment, user, balance;
        if (this.dataIncome[i].category != null) {
          category = this.dataIncome[i].category.name;
        }
        if (this.dataIncome[i].payment != null) {
          payment = this.dataIncome[i].payment.name;
        }
        if (this.dataIncome[i].user != null) {
          user = this.dataIncome[i].user.first_name;
        }
        if (this.dataIncome[i].balance != null) {
          balance = this.dataIncome[i].balance.name;
        }
        exp.push({
          Tanggal: this.dataIncome[i].created_at,
          NamaTransaksi: this.dataIncome[i].name,
          Nominal: this.dataIncome[i].amount,
          NamaPengguna: user,
          Kategori: category,
          Kas: balance,
          Pembayaran: payment,
          Deskripsi: this.dataIncome[i].description,
          Catatan: this.dataIncome[i].note,
        });
      }
      const data = XLSX.utils.json_to_sheet(exp);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, data, "Pemasukan");
      XLSX.writeFile(wb, "DataPemasukan.xlsx");
    },
    toastAlert(act) {
      this.$swal({
        toast: true,
